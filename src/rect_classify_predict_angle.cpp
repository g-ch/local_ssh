...
//
    ...
    if(if_training_gateway_position){
        ...
        train_lenet(nn, training_data_desired_form, training_labels_desired_form, validation_data_desired_form,
                validation_labels_desired_form, 1.0, 300, 32, "LeNet-model-rects-regression");
...
//            float dx2 = cos(result_angles[i]);
//            float dy2 = sin(result_angles[i]);
//            float scalar_multiply = dx1 * dx2 + dy1 * dy2;
//
//            if(scalar_multiply > 0){
//                std::cout << "correct direction" << result_angles[i];
//                result_angles[i] -= CV_PI;
//                std::cout <<" to " << result_angles[i] << std::endl;
//            }
//        }
